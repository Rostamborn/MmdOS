...
execution_context* syscall_handler(execution_context* frame) {
    uint64_t (*handler)(uint64_t  frame
                        , uint64_t a0
                        , uint64_t a1
                        , uint64_t a2
                        , uint64_t a3);
    ...
    frame->rax = handler((uint64_t)frame
                         , frame->rsi
                         , frame->rdx
                         , frame->rcx
                         , frame->r8);
...
uint64_t _syscall_memcpy(uint64_t frame, uint64_t dest, uint64_t src, uint64_t size, uint64_t unused) {
...
// asm ("int $128" : :: "rdi", "rsi", "rdx", "rcx", "r8", "memory");
// }

uint64_t syscall_memcpy(void* dest, void* src, uint64_t size) {
    uint64_t res = do_syscall(0, (uint64_t)dest, (uint64_t)src, (uint64_t)size, 0);
    return res;
}
