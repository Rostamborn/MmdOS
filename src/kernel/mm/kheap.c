...
void* k_heap_alloc(uint64_t size) {
    ...
    for (int i = 0; i < pagemap->arena_count; i++) {
        ...
        if (arena_offset + size + metadata_size <= arena_size) {
            k_heap_metadata* metadata = (k_heap_metadata*)(arena_base + arena_offset);
    ...
    if (current != NULL) {
        for (;current != NULL;) {
            if (current->next == NULL) {
                current->next = (vm_arena*)((void*)new_arena + HHDM_OFFSET);
                pagemap->arena_count++;
                break;
            }
        ...
        pagemap->arenas = (vm_arena*)((void*)new_arena + HHDM_OFFSET);
        pagemap->arena_count = 1;
    }
    spinlock_release(&pagemap->lock); 
        
    if (!vmm_map(pagemap, (uintptr_t)((void*)new_arena + HHDM_OFFSET), (uintptr_t)new_arena, new_arena->flags)) {
    ...
    metadata->arena = (vm_arena*)((void*)new_arena + HHDM_OFFSET);
...
void k_heap_free(void* ptr) {
    k_heap_metadata* metadata = (k_heap_metadata*)((uintptr_t)ptr - sizeof(k_heap_metadata));
    ...
    if (arena->allocated == 0) {
        ...
        if (current == arena) {
            ...
            vmm_kernel->arenas = arena->next;
            pmm_free((void*)vmm_virt2phys(vmm_kernel, (uintptr_t)arena, false), DIV_ROUNDUP(arena->size, PAGE_SIZE) + 1);
