    ...
    for (uint64_t i = 0; i < GOL_WIDTH; i++) {
        for (uint16_t j = 0; j < GOL_HEIGHT; j++) {
            ...
            if (i == 0 || i == GOL_WIDTH-1 || j == 0 || j == GOL_HEIGHT-1) {
                line[j] = GOL_BORDER;
            } else if ((gol_cursor[0] != 0 || gol_cursor[1] != 0) && gol_cursor[0] == i && gol_cursor[1] == j) {
                line[j] = GOL_CURSOR;
            } else if (cells[index]) {
                line[j] = GOL_CELL;
            }  else {
...
void gol_key_handler(uint8_t* cells, uint8_t* cursor, uint8_t* state) {
    ...
    switch (key) {
        ...
        case 'd':
            ...
            break;
        case 'e':
            cells[GOL_CI(cursor[0], cursor[1])] = 1;
            // !cells[GOL_CI(cursor[0], cursor[1])].alive;
            break;
        case 'f':
            if (*state == 0) {
                *state = 1;
